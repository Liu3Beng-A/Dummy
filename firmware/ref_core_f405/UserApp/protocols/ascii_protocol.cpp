...
void OnUsbAsciiCmd(const char* _cmd, size_t _len, StreamSink &_responseChannel)
{
    ...
    if (_cmd[0] == '!')
    {
        ...
        else if (s.find("STALL_RESUME") != std::string::npos)
        {
            ...
            // 解除 LOCKED：ClearStallMode 内部会下发 Enable 让电机端 ClearStallFlag
            dummy.ClearStallMode(targetIdx);
    ...
    if (_cmd[0] == '>' || _cmd[0] == '@' || _cmd[0] == '&' || _cmd[0] == '$')
    {
         ...
         * 任意电机处于 LOCKED → 直接拒绝运动指令，提示先 !STALL_RESUME */
        if (dummy.IsAnyMotorStalled())
        {
            Respond(_responseChannel, "error: motor stalled, send !STALL_RESUME first");
            return;
        }
