...
void Motor::CloseLoopControlTick()
{
    ...
    } else
    {
        switch (controller->modeRunning)
        {
            ...
            case MODE_COMMAND_POSITION:
                // LOCKED 期间丢弃位置命令 (已在 softPosition 设置阶段过滤)
                controller->CalcDceToOutput(controller->softPosition, controller->softVelocity);
                break;
        ...
        controller->stallMode == STALL_IDLE)
    {
        ...
        else
        {
            ...
            if (allConditionsMet)
            {
                ...
                else
                {
                    ...
                    if (controller->stallStartTick >= stallDurationUs)
                    {
                        ...
                        {
                            // 二次验证通过: IDLE -> RETREATING
                            controller->retreatStartTick = HAL_GetTick();

                            // 计算回退方向和目标 (向减小误差的反方向退)
                            controller->retreatDirection = (controller->goalPosition - controller->estPosition) > 0 ? -1 : +1;
                            controller->retreatTarget = controller->estPosition + controller->retreatDirection * 35556; // 关节: 35556 步

                            // 以当前位置为新任务起点, 开始梯形回退
                            motionPlanner.positionTracker.NewTask(controller->estPosition, controller->estVelocity);
                            // 设置回退速度 50000 step/s
                            config.motionParams.ratedVelocity = 50000;
                            motionPlanner.positionTracker.SetVelocityAcc(100000); // 100000 step/s²
                            ...
                            // 立即发第 1 次 STALL 广播 (Q4) - 设置标志，100Hz 中断里发
                            controller->stallBroadcastCmd = 1; // STALL_TRIGGER
                        }
                        else if (!velocityStillLow)
                        {
                            // 200ms 后速度起来了 → 不是堵转, 是暂时挤压后脱困
                            // 重置所有状态
                            controller->stallDetectRisingEdge = false;
                            controller->stallStartTick = 0;
                        }
                        // 否则（电流下降但误差仍在）：可能是堵转但电流在回落，
                        //   继续累积，等到 stallStartTick 翻倍后强制触发或保持状态
                        else if (controller->stallStartTick >= stallDurationUs * 2)
                        {
                            // 400ms 兜底: 即使条件 B/C 不满足, 也强制触发（保险机制）
                            controller->retreatStartTick = HAL_GetTick();
                            controller->retreatDirection = (controller->goalPosition - controller->estPosition) > 0 ? -1 : +1;
            ...
            else
            {
                ...
                // 防止小幅抖动 (estVelocity 在 30 周围跳动) 反复清零
                ...
                // 否则保持当前状态，等待电机真正动起来或堵转持续
            }
        }
    }

    /******************************** RETREATING State Handler *******************************/
    // 放在 Update State 之后：用 Update State 写出的 state 做判定。
    // (Update State 中 RETREATING/LOCKED → state=STATE_STALL, 只有 FINISH/RUNNING 才进 Update State;
    //  所以此处若 softPosition==goalPosition 且 softVelocity==0 即视为回退到位, 转 LOCKED)
    if (controller->stallMode == STALL_RETREATING)
    {
        ...
        if (reachedGoal)
        {
            // 回退完成: RETREATING -> LOCKED
            controller->stallMode = STALL_LOCKED;
            // 清除积分 (Q1-A1)
            controller->ClearIntegral();
            // 发 STALL_DONE 广播 (Q4) - 设置标志，100Hz 中断里发
