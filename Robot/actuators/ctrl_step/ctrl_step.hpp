...
public:
    ...
    void SetPositionDirect(float _targetPos);
    // CAN Command
    ...
    // 广播急停（静态方法，发送至 nodeID=0 广播地址）
    static void BroadcastEmergencyStop(CAN_HandleTypeDef* _hcan);

    void UpdateAngle();
    void UpdateAngleCallback(float _pos, int16_t _state, uint8_t _flag1, uint8_t _flag2);
